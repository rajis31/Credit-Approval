...
1. **Business Problem:** We want to be able to accuartely predict whether or not an applicant should be approved or not.
2. **Data Preparation** The data has already been gathered and put together in a table format. Here we will load into R and take a look some general attributes and exploratory work related to the dataset before we start building predictive models.
...
```{r}
paste0("The shape of the dataset is (",nrow(data),",",ncol(data),")"  )
```

 

The columns are anonymized since this is actual applicant information. Here we rename the columns of the dataset.
...
Now we will convert the data to the appropriate data types and recode the target variable to 0 for not approved and 1 for approved. We then use the `as.numeric` function to convert the characters to the numerical factors.
...
This dataset does have missing values which were imputed beforehand. We will check for column wise just to be on the safe side. Missing values are coded as ?. We have missing values in columns V2 and V14 which are both continous, and we will impute these with the mean.
...
**Plot 2** shows the marginal distribution of the target variable vs V4. Some values are more weighted towards approved while others are more towards rejected.

 

```{r}

p<-ggplot(data=data, aes(x=V4,y=Target)) +geom_boxplot()

p<-p+ggtitle("Marginal Distribution of V4 vs Target")+
...
Now we will split apart the dataset into the respective subsets.
...
Train K-NN Model first as shown below, and then fine tune the model using the validation set. We will then create the confuusion matrix which shows the number of correct/wrong predicted values.
...
test.pred<-knn(train.x,test.x,cl=train.y, k=5)
tab1<-table(test.y ,test.pred)
print(paste0("The misclassification rate on the training set is ", round((tab1[2]+tab1[3])/nrow(test.x),2)*100," %"))
```
